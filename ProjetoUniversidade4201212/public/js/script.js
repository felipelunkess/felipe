...
function generateFormHTML(entityType, data) {
  ...
  formHTML += `
    ...
    <div class="form-group">
      ...
      <select id="id_departamento" name="id_departamento" required>
        <option value="">Selecione um departamento</option>
        ${currentData.departamentos.map(d => `
    ...
    <div class="form-group">
      ...
      <select id="id_turma" name="id_turma" required>
        <option value="">Selecione uma turma</option>
        ${currentData.turmas.map(t => {
          const curso = currentData.cursos.find(c => c.id_curso === t.id_curso);
          return `<option value="${t.id_turma}" ${data?.id_turma == t.id_turma ? "selected" : ""}>
            ${curso?.descricao || "N/A"} - ${t.periodo} - ${t.turno}
          </option>`;
        }).join("")}
      </select>
    </div>
    ...
    <div class="form-group">
      ...
      <select id="id_curso" name="id_curso" required>
        <option value="">Selecione um curso</option>
        ${currentData.cursos.map(c => `
          <option value="${c.id_curso}" ${data?.id_curso == c.id_curso ? "selected" : ""}>
            ${c.descricao}
          </option>`).join("")}
      </select>
    </div>
...
function filtrarPorTurno() {
  const turnoFiltro = document.getElementById('filter-turno-select').value.toLowerCase();
  const tbody = document.getElementById('tbody-turma');
  const linhas = tbody.getElementsByTagName('tr');

  for (let i = 0; i < linhas.length; i++) {
    const celulaTurno = linhas[i].getElementsByTagName('td')[4]; // coluna Turno
    if (celulaTurno) {
      const turno = celulaTurno.textContent.toLowerCase();
...
function filtrarPorStatus() {
  ...
  const statusSelecionado = select.value.toLowerCase();
  const tbody = document.getElementById('tbody-aluno');
  const linhas = tbody.getElementsByTagName('tr');

  for (let i = 0; i < linhas.length; i++) {
    const linha = linhas[i];
    const status = linha.cells[5].textContent.toLowerCase(); // coluna Status
...
function filtrarPorStatusMatricula() {
  const select = document.getElementById('filter-matricula-status-select');
  const statusSelecionado = select.value.toLowerCase();
  ...
  for (let i = 0; i < linhas.length; i++) {
    const linha = linhas[i];
    const status = linha.cells[4].textContent.toLowerCase(); // coluna Status
    if (statusSelecionado === '' || status === statusSelecionado) {
      linha.style.display = '';
    } else {
      linha.style.display = 'none';
...
function filtrarPorStatusPagamento() {
  const select = document.getElementById('filter-pagamento-status-select');
  const statusSelecionado = select.value.toLowerCase();
...
if (loginForm) {
  loginForm.addEventListener('submit', async (e) => {
    ...
    if (!username || !password) {
      loginMessage.textContent = 'Preencha username e password.';
      return;
    }
    try {
    ...
    loginButton.addEventListener('click', async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          alert(data.message || 'Falha no login. Verifique usuário e senha.');
        }
      } catch (err) {
