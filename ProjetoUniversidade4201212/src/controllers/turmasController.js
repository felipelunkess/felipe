...
const listarTurmas = async (req, res) => {
  const turno = req.query.turno; // recebe ?turno=diurno ou noturno

  try {
    const turmas = await Turma.listarComFiltro(turno);
    res.render('turmas', { turmas });  // Ajuste para a view que usa
  } catch (error) {
